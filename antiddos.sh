This is a UNIX style bash script, save it anywhere on your linux host machine and start it automatically with your server..
...
# Allow outgoing fnf requests. Unencrypted, use with care.
$IPTABLES -A OUTPUT -m state --state NEW -p tcp --dport 30120 -j ACCEPT
 
# Allow outgoing tx requests.
$IPTABLES -A OUTPUT -m state --state NEW -p tcp --dport 40120 -j ACCEPT
 
# Allow outgoing sql requests. Do NOT allow unencrypted sql!
